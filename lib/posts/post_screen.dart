...
class _PostScreenState extends State<PostScreen> {
 ...
 late String choice;
  ...
  Widget _content(int index ,String desc ,String text , String id ,String image) {
    ...
    ImageChunkEvent? loadingProgress) {
              ...
              errorBuilder: (BuildContext context,Object exception,StackTrace? stacktrace){
                return Center(child: const Text("Due to network issue image can't be loaded"));
              },)
          ),
          SizedBox(height: 10,),
          Row(
            ...
            children: [
              (desc == 'null' || desc.isEmpty) ? richtext(
                  'Description', 'No description of this image') : Expanded(
                    child: richtext(
                    'Description', '$desc'),
                  ),
  ...
  void _updateImage(String id,String imagechoice){
    ...
    Navigator.push(context,MaterialPageRoute(builder: (context) =>PhotoAccess(Id: id,choice: imagechoice)));
  }

 Future<String>  _profile() async{
   DatabaseEvent event = await delref.child('${_auth.currentUser?.uid}').child('Current_User_Info').child('ProfileImage').once();
